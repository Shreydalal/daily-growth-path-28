...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Sign In — Transformation Tracker" },
      { name: "description", content: "Sign in to your personal transformation tracker to sync habits, goals, and stats." },
    ],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  const { user, login, verifyOtp, loginWithGoogle, loading } = useAuth();
  ...
  // Sync state resolving: wait 500ms for Supabase local storage session to hydrate
  useEffect(() => {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-muted/30 px-4">
      <div className="w-full max-w-md p-8 bg-card border rounded-2xl shadow-xl mx-4 space-y-6">
        {!isConfigured ? (
          <div className="space-y-4 pt-2">
            ...
            <div className="text-center">
              <h2 className="text-xl font-bold">Cloud Sync Setup Required</h2>
              <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                Supabase configuration variables are not set yet. To enable authentication, you will need to:
              </p>
            </div>
          ...
          <div className="space-y-5 pt-2">
            <div className="text-center">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-semibold mb-2">
                <Sparkles className="size-3" /> Transformation Tracker
              </div>
              <h2 className="text-2xl font-bold">Welcome Back</h2>
              <p className="text-sm text-muted-foreground mt-1">
                Your 6-month journey to career and life excellence.
              </p>
            </div>
            ...
            {step === "email" ? (
              <div className="space-y-4">
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2.5 py-3 rounded-xl border bg-background hover:bg-muted font-semibold transition-colors disabled:opacity-50 text-sm cursor-pointer"
                >
                ...
                <div className="relative my-4">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-card px-2 text-muted-foreground font-medium">Or email code</span>
                  </div>
                </div>
                ...
                <form onSubmit={handleSendEmail} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold hover:bg-primary/90 transition-colors disabled:opacity-50 text-sm cursor-pointer"
                  >
                    {loading ? (
                      <>
